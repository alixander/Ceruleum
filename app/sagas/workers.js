...
export function* onSetRootDirectory() {
  try {
    const folderName = yield call(selectFolder);
    yield put({ type: 'SET_ROOT_DIRECTORY', data: folderName });
    yield put({ type: 'REFRESH_FILES_INTENT' });
  } catch (e) {
    // Do nothing. User cancelled request to set root directory
  }
}

// Auto-display the first file of a new directory if none already set
function* displayFirstFile(root) {
  const isAnyFileDisplayed = yield select(Utils.isAnyFileDisplayed);
  if (isAnyFileDisplayed) {
    return;
  }
  ...
  yield call(displayFile, { data: currentNode });
}
...
export function* refreshFiles() {
  ...
  try {
    ...
    yield call(displayFirstFile, parsedFilenamesRoot);
    yield put({
      type: 'REFRESH_FILES',
